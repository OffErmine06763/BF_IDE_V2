...
namespace BFC
{
	...
	public:
		...
		coord<u32> Position(const u32 id) { return tr.positions.at(id); }
	...
	static expected<Loop, CompilerError> ParseLoop(Parser& parser, TU& tu)
	{
		...
		while (!parser.Done())
		{
			...
			case T_INC: case T_DEC: case T_LEFT: case T_RIGHT: case T_I: case T_O:
				body.push_back({ Operation{ OpFromTType((TType)token.type), to<u16>(token.count) } });
				parser.MoveToNextToken();
				break;
			case T_LOOPS:
				parser.Back();
	...
	static expected<Block, CompilerError> ParseRoot(Parser& parser, TU& tu)
	{
		...
		while (!parser.Done())
		{
			...
			{
			case T_INC: case T_DEC: case T_LEFT: case T_RIGHT: case T_I: case T_O:
				block.items.push_back({ Stmt{ Operation{ OpFromTType((TType)token.type), to<u16>(token.count) } } });
				parser.MoveToNextToken();
				break;
			case T_LOOPS:
				parser.Back();
	...
	// #################################################################### OPTIMIZE ###########################################################################
	/// Represents the result of merging two statements
	enum class MergeResult : u8
	{ 
		NOOP, /// No merging happened
		HALF, /// Two statements have been merged in a single one
		FULL  /// The two statements erase each other
	};
	/// Merge the two given statements, if possible.
	/// If they elide each other, no change is made to them.
	/// If only one can be erased, then 'prev' will be updated with the resulting statment, while 'stmt' can be discarded.
	static MergeResult Merge(Stmt& prev, Stmt& stmt)
	{
		if (holds<Operation>(stmt.value, prev.value))
		{
			...
			if (op.type + prevo.type == 0) // erase opposite operations
			{
			...
			else if (op.type == prevo.type) // merge identical operations (previous erasures might allow more merging)
			{
				if (op.count + prevo.count + 1 <= MAX_COUNT)
				{
					// +++ ++++ == +++++++
					//   2 +  3 =   6
					// 200 + 54 = 255
				...
				else
				{
					// 200 + 55 = 255 + 0
					// 200 + 56 = 255 + 1
					u16 old = (op.count + prevo.count) - MAX_COUNT;
					prevo.count = MAX_COUNT;
					op.count = old;
	...
	// #################################################################### COMPILE ###########################################################################
	static CompilerError ParseArgs(CompilationParams& p, const std::vector<std::string>& args)
	{
		for (size_t i = 1; i < args.size(); i++)
		{
			...
			else if (arg == "-o" || arg == "-out" || arg == "--out")
			{
				...
				p.intermediates = CompilationParams::OBJ;
			else if (arg == "-all" || arg == "--all")
				p.intermediates = CompilationParams::ALL;
			else if (arg == "-i" || arg == "-inter" || arg == "--inter")
			{
				if (i == args.size() - 1)
					return Compiler::GetIntermediatePathError(arg);
				p.intermediatesPath = args[++i];
			}
			...
			else if (arg == "-phase" || arg == "--phase")
			{
				if (i == args.size() - 1)
					...
					p.tgtPhase = CompilationParams::TOKEN;
				else if (args[i] == "parse")
					p.tgtPhase = CompilationParams::PARSE;
				else if (args[i] == "analyze")
					p.tgtPhase = CompilationParams::ANAL;
				else if (args[i] == "optimize")
					p.tgtPhase = CompilationParams::OPT;
				else if (args[i] == "intermediate")
					p.tgtPhase = CompilationParams::INTER;
				else
