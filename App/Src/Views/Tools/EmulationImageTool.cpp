...
void EmulationImageTool::OnOutput()
{
	...
	// read 16x16 cells as RGB332 colors
	...
	CreateImage(rgbaPixels);
}
void EmulationImageTool::CreateImage(const std::vector<u8> rgbaPixels)
{
	...
	App::D3D12Allocator->Alloc(&my_texture_srv_cpu_handle, &my_texture_srv_gpu_handle);
	CreateTexture(rgbaPixels.data(), App::D3D12Device, my_texture_srv_cpu_handle, &tex_resources);
}

void EmulationImageTool::Render()
{
	ImGui::Image((ImTextureID)my_texture_srv_gpu_handle.ptr, ImVec2(128, 128));
}