...
#define CREATE_BUS_FORWARDING_SUB(EventBus, Prop, m_EventBus) \
	...
	template <typename C> requires InVariant<C, EventBus::payload_t>																	\
...
public:
	...
	template <typename C> requires InVariant<C, payload_t>
	listener_id Subscribe(const prop_id key, consumer<C> cb)
	{
		return m_Listeners[key].Subscribe(CreateListener(cb));
	}
	listener_id Subscribe(const prop_id key, callable cb) requires InVariant<std::monostate, payload_t>
	{
		return m_Listeners[key].Subscribe(CreateListener(cb));
	}
	...
	void Notify(const prop_id key, const payload_t& data = {}) requires InVariant<std::monostate, payload_t>
	{
	...
	static consumer<payload_t> CreateListener(const callable& cb) requires InVariant<std::monostate, payload_t>
	{
		return [cb](payload_t p) {
